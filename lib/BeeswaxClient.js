...
var entities = {
    ...
    // ! targeting routes only support queryAll()
    targetingContent: {
        endpoint: '/rest/targeting/content'
    },
    targetingDomain: {
        endpoint: '/rest/targeting/domain'
    },
    targetingGeo: {
        endpoint: '/rest/targeting/geo'
    },
    targetingInventory: {
        endpoint: '/rest/targeting/inventory'
    },
    targetingIpAddress: {
        endpoint: '/rest/targeting/ip_address'
    },
    targetingMobileApp: {
        endpoint: '/rest/targeting/mobile_app'
    },
    targetingPlatform: {
        endpoint: '/rest/targeting/platform'
    },
    targetingSegment: {
        endpoint: '/rest/targeting/segment'
    },
    targetingTime: {
        endpoint: '/rest/targeting/time'
    },
    view: {
        endpoint: '/rest/view'
    }
...
BeeswaxClient.prototype._create = function(endpoint, idField, body, id) {
    id = id || false;
    var self = this;
...
BeeswaxClient.prototype.segmentUpload = function(params){
    var self = this;

    function init(params){
        var segmentDef = {},
            props = ['file_name','size_in_bytes','segment_key_type','user_id_type'];
        ...
        if (params.sourceUrl && params.segmentContentBytes) {
            return Promise.reject(
                new Error('segmentUpload can only accept one source parameter' +
                    ' at the time (either sourceUrl or segmentContentBytes).')
            );
        }
        ...
        if (!(params.sourceUrl || params.segmentContentBytes)) {
            return Promise.reject(
                new Error(
                    'segmentUpload params requires a sourceUrl' +
                    ' or a segmentContentBytes property.'
                )
            );
        }


        if (!segmentDef.file_name){
            segmentDef.file_name =
                urlUtils.parse(params.sourceUrl).pathname.split('/').pop();
        }
    ...
    function getSize(data){
        return new Promise(function(resolve,reject){
            ...
            // well, if it's a byte-array we already know it's size
            if (data.req.segmentContentBytes) {
                data.segmentDef.size_in_bytes = data.req.segmentContentBytes.length;
                return resolve(data);
            } else {
    ...
    function createSegmentUpload(data) {
        var opts = {
            url: urlUtils.resolve(self.apiRoot, '/rest/segment_upload'),
            body: data.segmentDef
        };
    ...
    function postSegmentUploadFile(data){
        return new Promise(function(resolve,reject){
            var opts = {
                url: urlUtils.resolve(self.apiRoot, '/rest/segment_upload/upload/' +
                    data.createResponse.payload.id ),
                jar : self._cookieJar
            };
            ...
            var formOptions = null
            if (data.req.sourceUrl) {
                segmentData = request(data.req.sourceUrl);
            } else if (data.req.segmentContentBytes) {
                segmentData = data.req.segmentContentBytes;
                formOptions = {filename: data.req.file_name};
            } else {
                return reject(
                    new Error('Can\'t find any parameters containing segment content data')
                );
            }

            form.append('segment_file', segmentData, formOptions);
        });
    }

    function getSegmentUpload(data){
        var opts = {
            url: urlUtils.resolve(self.apiRoot, '/rest/segment_upload/' +
                data.postFileResponse.payload.id )
        };
    ...
    return init(params)
        .then(getSize)
        .then(createSegmentUpload)
        .then(postSegmentUploadFile)
        .then(getSegmentUpload);
};
